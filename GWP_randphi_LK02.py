...
#--------------------------------------------------#
# First part : calculate the sea-surface elevation #
#--------------------------------------------------#
...
# Draw randomly the value of the uniform phase between -Pi and Pi
phi0=(1-2*np.random.rand(1))*np.pi
...
eta1=np.zeros((Nx,Nt),float)                
for i in range(Nx):
    for k in range(Nt):
        eta1[i,k]=np.sum(a*np.cos((x[i]+cg0*t[k])*kx+phi-t[k]*omega))
...
tfoc3=np.zeros(Nt-3)
tau_abs3=np.zeros(Nt-3)
A_ratio3=np.zeros(Nt-3)
L3=np.zeros(Nt-3)
...
# Store the data
np.savez('GWP_2D_Lk02_phi%.5f' % phi0,tfoc1=tfoc1,tau_abs1=tau_abs1
